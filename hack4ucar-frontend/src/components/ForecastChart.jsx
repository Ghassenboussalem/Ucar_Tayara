...
function CustomTooltip({ active, payload, label, lang = 'fr' }) {
  ...
  return (
    <div style={{ background: 'white', border: '1px solid #e2e8f0', borderRadius: '10px', padding: '10px 14px', boxShadow: '0 4px 16px rgba(0,0,0,0.1)', fontSize: '0.8rem' }}>
      <div style={{ fontWeight: 700, color: '#0f172a', marginBottom: '6px', display: 'flex', alignItems: 'center', gap: '6px' }}>
        {label}
        {isForecast && <span style={{ padding: '1px 6px', borderRadius: '4px', background: 'rgba(29,83,148,0.1)', color: 'rgb(29,83,148)', fontSize: '0.65rem', fontWeight: 700 }}>{tx('PRÉVISION', 'توقع')}</span>}
      </div>
  ...
  if (!data || data.length === 0) {
    return (
      <div style={{ height: 260, display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#94a3b8', fontSize: '0.82rem' }}>
        {tx('Données insuffisantes pour la prévision', 'بيانات غير كافية للتوقع')}
      </div>
    )
  }
      ...
      {lastActual != null && (
        <div style={{ display: 'flex', gap: '16px', flexWrap: 'wrap' }}>
          <div style={{ padding: '8px 14px', borderRadius: '8px', background: '#f8fafc', border: '1px solid #e2e8f0' }}>
            <div style={{ fontSize: '0.65rem', color: '#94a3b8', fontWeight: 600, textTransform: 'uppercase', marginBottom: '3px' }}>{tx('Actuel', 'الحالي')}</div>
          ...
          <div style={{ padding: '8px 14px', borderRadius: '8px', background: trendColor + '08', border: `1px solid ${trendColor}30` }}>
            <div style={{ fontSize: '0.65rem', color: '#94a3b8', fontWeight: 600, textTransform: 'uppercase', marginBottom: '3px' }}>{tx('Prévu (S2 2026)', 'متوقع (س2 2026)')}</div>
          ...
          {changePct != null && (
            <div style={{ padding: '8px 14px', borderRadius: '8px', background: '#f8fafc', border: '1px solid #e2e8f0' }}>
              <div style={{ fontSize: '0.65rem', color: '#94a3b8', fontWeight: 600, textTransform: 'uppercase', marginBottom: '3px' }}>{tx('Variation', 'التغير')}</div>
          ...
          {confidence != null && (
            <div style={{ padding: '8px 14px', borderRadius: '8px', background: 'rgba(29,83,148,0.05)', border: '1px solid rgba(29,83,148,0.15)' }}>
              <div style={{ fontSize: '0.65rem', color: '#94a3b8', fontWeight: 600, textTransform: 'uppercase', marginBottom: '3px' }}>{tx('Confiance', 'الثقة')}</div>
      ...
      <ResponsiveContainer width="100%" height={260}>
        <ComposedChart data={data} margin={{ top: 5, right: 5, left: 0, bottom: 5 }}>
          ...
          <Area
            type="monotone"
            dataKey="upper"
            name={tx('Borne haute', 'الحد الأعلى')}
            stroke="none"
            fill={color}
            fillOpacity={0.08}
            legendType="none"
          ...
          <Area
            type="monotone"
            dataKey="lower"
            name={tx('Borne basse', 'الحد الأدنى')}
            stroke="none"
            fill="white"
            fillOpacity={1}
            legendType="none"
          ...
          {splitName && (
            <ReferenceLine
              ...
              stroke="#94a3b8"
              strokeDasharray="4 2"
              label={{ value: tx('▶ Prévision', '▶ توقع'), position: 'insideTopRight', fontSize: 10, fill: '#94a3b8', dy: -4 }}
            />
          )}
        </ComposedChart>
      </ResponsiveContainer>
